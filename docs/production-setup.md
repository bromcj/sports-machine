...
- dev **has its own `data/`** — a separate database, not production's. The
  ...
  point it elsewhere.
- `python -m pytest tests/ -q` passes: 643 passed, 2 skipped on a checkout
  without `data_golden/` (the two skipped are the golden test), and 645
  passed in the dev folder, which has it (both measured 2026-09-25 on the
  `phase-a-foundations` branch).
...
You should see everything pass (643 passed, 2 skipped on 2026-09-25 — the two
